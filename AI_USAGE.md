...
- **Proxy do frontend nunca alcançava a API dentro do Compose**: `docker-compose.yml` passava `API_INTERNAL_URL: http://api:8080` como `environment:` (runtime) do serviço `frontend`. Só apareceu ao subir `db` + `api` + `frontend` juntos pela primeira vez: o `rewrites()` do Next.js (repo `srm-frontend`) é resolvido em *build-time* e fica congelado no `routes-manifest.json` do output standalone — variável setada em runtime nunca chega lá, e o proxy caía sempre em `ECONNREFUSED 127.0.0.1:8080` dentro do container. Corrigido movendo o valor para `build.args` do serviço `frontend`, casado com `ARG`/`ENV` no `Dockerfile` do outro repo antes do `next build`. Confirmado ao vivo: `curl http://localhost:3000/api/v1/tipos-recebivel` retornando o payload real da API via proxy.
